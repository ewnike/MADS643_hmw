...
bar_type = np.dtype(
    [
        ("date", "M8[D]"),
        ("time", "m8[us]"),  # store date in months units, with day resolution
        ("open_p", "f8"),
        ("high_p", "f8"),  # float (decimal)
        ("low_p", "f8"),
        ("close_p", "f8"),
        ("per_vlm", "u8"),
    ]
...
h5_bar_type = np.dtype(
    [
        ("date", "i8"),
        ("time", "i8"),  # store date as just a long decimal number.
        ("open_p", "f8"),
        ("high_p", "f8"),  # float (decimal)
        ("low_p", "f8"),
        ("close_p", "f8"),
...
def ticks_to_bars(tick_data, interval):
    ...
    begin_time = tick_data[0]["date"] + start_us.astype(
        "m8[us]"
    )  # interval start at first tick
